def es_primo(numero): # se corregio porque le faltaba los dos puntos 
    if numero < 2: # se define la variable numero 
        return False
    for i in range(2, int(numero**0.5) + 1):# se agregaron los dos puntos al final
        if numero % i == 0:
...
primos = [num for num in range(2, limite + 1) if es_primo(num)]# se agrega num en la funcion de la variable
